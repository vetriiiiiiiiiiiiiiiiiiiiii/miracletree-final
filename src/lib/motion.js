...
export function splitLines(element) {
  ...
  const lines = [];
  element.innerHTML = "";
  for (const bucket of [...rows.entries()]
    .sort((a, b) => a[0] - b[0])
    .map((e) => e[1])) {
 ...
 */
export function imageReveal(wrapper, image) {
  if (prefersReducedMotion()) {
 ...
 */
export function productRotation(trigger, onProgress) {
  if (prefersReducedMotion()) {
    onProgress(0);
    return undefined;
  }
 ...
 */
export function stickyProgress(track, onProgress, options = {}) {
  if (prefersReducedMotion()) {
...
export function treeGrowth(track, onProgress) {
 ...
 */
export function horizontalRail(track, rail) {
...
export function sectionTransition(overlay, direction) {
  if (prefersReducedMotion()) {
