...
import { orderConfirmation, newOrderAlert, type OrderMailData } from "@/lib/mail-templates";
...
async function load(orderNumber: string): Promise<OrderMailData | null> {
 ...
 */
export async function sendOrderPlacedEmails(orderNumber: string): Promise<void> {
 ...
 */
export function statusDeservesEmail(status: OrderStatus): boolean {
  return (
    status === "shipped" ||
