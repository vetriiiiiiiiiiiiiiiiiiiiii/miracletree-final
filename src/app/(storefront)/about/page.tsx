...
export default async function AboutPage() {
  const [milestones, accolades, credits, notes] = await Promise.all([
    prisma.milestone.findMany({ where: { isActive: true }, orderBy: { position: "asc" } }),
    prisma.accolade.findMany({ where: { isActive: true }, orderBy: { position: "asc" } }),
            ...
            <div className="grid gap-14 lg:grid-cols-[0.85fr_1.15fr]">
              <div>
                ...
                <div className="mt-8 hidden lg:block">
                  <BotanicalPlate subject="flower" className="h-56 w-40" label="blossom — also eaten" />
                </div>
              </div>
