...
import { COMPANY_FACTS } from "@/lib/leadership";
      ...
      <Section spacing="loose">
        <Container>
          ...
          <p className="mt-6 max-w-[52ch] text-cream-300">
            We are still writing these. The company history is on the story
          ...
          <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
            ...
            <div>
              <Reveal>
                ...
                {founder.credential ? (
                  <p className="mt-4 text-[1.1rem] text-cream-400">{founder.credential}</p>
                ) : null}
                {founder.bio ? (
              ...
              {founder.sourceUrl ? (
                <Citation source={founder.source} url={founder.sourceUrl} className="mt-8" />
              ) : null}
          ...
          <Reveal className="mt-16" stagger={0.1}>
            <div className="grid gap-px border border-border-subtle bg-border-subtle lg:grid-cols-2">
              {INTERNATIONAL.map((project) => (
                <article key={project.place} data-animate="fade-up" className="bg-ink p-8 md:p-12">
          ...
          <Container>
            ...
            <Reveal className="mt-16" stagger={0.08}>
              <ul className="grid gap-px border border-border-subtle bg-border-subtle md:grid-cols-2">
                {others.map((person) => (
                  <li key={person.id} data-animate="fade-up" className="bg-ink p-8 md:p-10">
                    ...
                    {person.bio ? (
                      <p className="mt-5 leading-relaxed text-cream-300">{person.bio}</p>
                    ) : null}

                    {person.highlights.length ? (
                      <ul className="mt-6 grid gap-2">
                        {person.highlights.map((h) => (
                          <li key={h.id} className="flex gap-3 text-[0.9rem] text-cream-400">
                            <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-emerald-400" aria-hidden />
                            <span>
                              {h.year ? (
                                <span className="tabular-nums text-gold-400">{h.year} — </span>
                              ) : null}
                              {h.title}
                            </span>
                          </li>
                    ...
                    {person.sourceUrl ? (
                      <Citation source={person.source} url={person.sourceUrl} className="mt-6" />
                    ) : null}
                  </li>
                ))}
              </ul>
