...
export async function placeOrderAction(formData) {
  ...
  if (input.paymentMethod === "razorpay" && !isRazorpayConfigured()) {
    return {
      ok: false,
      error:
        "Card and UPI payments are unavailable right now. Please choose cash on delivery.",
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error:
        "We couldn't reach the payment gateway. Try again, or choose cash on delivery.",
    };
  }
}
/**
...
export async function verifyPaymentAction(input) {
  ...
  if (!valid) {
    ...
    return {
      ok: false,
      error:
        "That payment could not be verified. You have not been charged twice — contact us if the amount was debited.",
    };
  }
