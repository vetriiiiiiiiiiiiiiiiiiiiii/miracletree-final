...
function guard(error: unknown): FormState {
...
export async function saveVariantAction(
  ...
  try {
    ...
    if (input.id) {
      ...
      await prisma.productVariant.update({ where: { id: owned.id }, data: variantData });
      await prisma.inventory.upsert({
        where: { variantId: owned.id },
        create: { variantId: owned.id, onHand: input.onHand, lowStockAt: input.lowStockAt },
