...
const LEADER_FIELDS: FieldSpec[] = [
  { name: "name", label: "Name", type: "text", required: true, maxLength: 160, half: true },
  {
    name: "role",
    label: "Role",
    type: "text",
...
export default async function LeadershipAdminPage() {
  ...
  const HIGHLIGHT_FIELDS: FieldSpec[] = [
    {
      name: "leaderId",
      label: "Person",
      type: "select",
    ...
    { name: "title", label: "What it is", type: "text", required: true, maxLength: 200 },
    { name: "body", label: "Detail", type: "textarea", rows: 3, maxLength: 2000 },
    { name: "position", label: "Order", type: "number", hint: "Lowest first, within that person." },
  ];
    ...
    <>
      ...
      <RecordManager
        ...
        records={leaders.map((l) => ({
          ...
          badges: [
            ...
            ...(l.isFounder ? [{ label: "Featured", tone: "info" as const }] : []),
            ...(l.highlights.length
              ? [{ label: `${l.highlights.length} on record`, tone: "neutral" as const }]
              : []),
            ...(l.sourceUrl ? [{ label: "Cited", tone: "info" as const }] : []),
          ],
          values: {
            name: l.name,
            role: l.role,
      ...
      <RecordManager
        ...
        records={highlights.map((h) => ({
          ...
          badges: [
            { label: h.leaderName, tone: "neutral" as const },
            { label: KIND_LABEL[h.kind] ?? h.kind, tone: "info" as const },
          ],
          values: {
            leaderId: h.leaderId,
            kind: h.kind,
