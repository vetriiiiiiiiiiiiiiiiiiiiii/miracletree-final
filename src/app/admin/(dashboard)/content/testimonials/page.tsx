...
const FIELDS: FieldSpec[] = [
  ...
  { name: "location", label: "Location", type: "text", half: true, placeholder: "Madurai" },
  {
    name: "body",
    label: "Quote",
    type: "textarea",
  ...
  return (
    <>
      <PageHeader
        ...
        breadcrumb={[{ label: "Content", href: "/admin/content" }, { label: "Testimonials" }]}
      />
      ...
      <Card className="mb-6">
        <p className="text-sm leading-relaxed text-cream-300">
          ...
          <Link href="/admin/reviews" className="underline underline-offset-4 hover:text-cream-50">
            Reviews
          </Link>
      ...
      <RecordManager
        ...
        records={testimonials.map((item) => ({
          ...
          badges: [
            { label: `${item.rating}/5` },
            ...(item.location ? [{ label: item.location, tone: "info" as const }] : []),
