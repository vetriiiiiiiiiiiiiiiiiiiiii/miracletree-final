  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Pill
            tone={user.role === "admin" ? "warning" : user.role === "staff" ? "info" : "neutral"}
          >
            {user.role}
          </Pill>
        }
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          hint={user.lastLoginAt ? `Last seen ${formatDate(user.lastLoginAt)}` : "Never signed in"}
        />
      </div>
        ...
        <div className="grid gap-6">
              ...
              {user.orders.length === 0 ? (
                ...
                user.orders.map((order) => (
                  <Tr key={order.id}>
                    ...
                    <Td align="center">
                      <Pill tone={order.paymentStatus === "paid" ? "success" : "neutral"}>
                        {order.paymentStatus}
                      </Pill>
                    </Td>
          ...
          {user.reviews.length ? (
            <Card title="Reviews" padded={false}>
              <Table
                head={["Product", { label: "Rating", align: "center" }, { label: "Status", align: "center" }, "Date"]}
              >
                {user.reviews.map((review) => (
                  <Tr key={review.id}>
                    ...
                    <Td className="text-xs text-cream-400">{formatDate(review.createdAt)}</Td>
                  </Tr>
                ))}
              </Table>
            </Card>
