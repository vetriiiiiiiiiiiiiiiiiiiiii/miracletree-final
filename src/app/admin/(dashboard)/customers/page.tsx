    ...
    <>
      ...
      <Card className="mt-8" padded={false}>
        ...
        >
          {users.length === 0 ? (
            <EmptyRow colSpan={6}>
              {params.q ? `No customers matched “${params.q}”.` : "No customer accounts yet."}
            </EmptyRow>
          ) : (
            users.map((user) => {
              const spend = user.orders.reduce((sum, order) => sum + order.grandTotal, 0);
              const name =
              ...
              return (
                <Tr key={user.id}>
                  ...
                  <Td>
                    ...
                    {user.phone ? (
                      <span className="mt-0.5 block text-xs text-cream-400">{user.phone}</span>
                    ) : null}
                  </Td>
