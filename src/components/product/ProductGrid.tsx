...
export function ProductGrid({
  ...
  className?: string;
  /**
   * Names the grid for assistive tech. Product names are `h3`, so a page whose
   * nearest ancestor heading is the `h1` needs this to avoid an h1→h3 skip.
   */
  headingLabel?: string;
}) {
  ...
  useEffect(() => {
    ...
    const cards = [...node.querySelectorAll<HTMLElement>("[data-animate]")];
  ...
  return (
    <section
      ref={ref}
      className={cn("grid gap-x-6 gap-y-14 md:gap-x-8", gridColumns[columns], className)}
