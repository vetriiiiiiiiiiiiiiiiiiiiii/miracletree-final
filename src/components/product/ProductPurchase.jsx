...
export function ProductPurchase({
  ...
  const stock = useMemo(() => {
    if (!selected) return { level: "out", text: "Unavailable" };
    if (!selected.tracked) return { level: "in", text: "In stock" };
    if (selected.available <= 0) return { level: "out", text: "Out of stock" };
    if (selected.available <= 5)
      return { level: "low", text: `Only ${selected.available} left` };
    return { level: "in", text: "In stock" };
  }, [selected]);
...
function TrustRow({ icon, children }) {
  const paths = {
