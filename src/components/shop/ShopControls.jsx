...
export function ShopToolbar({ total, groups, activeCount }) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-y border-border-subtle py-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 border border-border-subtle px-4 py-2 text-[0.68rem] uppercase tracking-[0.14em] text-cream-200 transition-colors hover:border-border-strong hover:text-cream-50 lg:hidden"
          >
            Filters
            {activeCount > 0 ? (
          ...
          <label className="flex items-center gap-2 text-[0.68rem] uppercase tracking-[0.14em] text-cream-400">
            ...
            <select
              ...
              className="border border-border-subtle bg-transparent py-2 pl-3 pr-8 text-[0.68rem] uppercase tracking-[0.14em] text-cream-100 focus:border-emerald-400 focus:outline-none"
...
function ActiveChips({ groups, searchParams, setParam, onClear }) {
  const chips = [];
    ...
    <div className="flex flex-wrap items-center gap-2 py-4">
      {chips.map((chip) => (
        <button
          ...
          className="inline-flex items-center gap-2 border border-border-strong px-3 py-1.5 text-xs text-cream-200 transition-colors hover:border-border-strong"
        >
          {chip.label}
...
export function ShopFilters({ groups }) {
...
function FilterGroup({ title, options, value, onSelect }) {
  ...
  return (
    <fieldset>
      <legend className="eyebrow mb-4 text-gold-400">{title}</legend>
  ...
  return (
    <label className="flex cursor-pointer items-center justify-between gap-4 text-sm text-cream-300">
      ...
      <button
        ...
        className={cn(
          // The switch stays visually small, but a transparent ::before pushes
          // the actual hit area out to a comfortable tap target.
          "relative h-5 w-9 shrink-0 border transition-colors duration-300",
          "before:absolute before:-inset-2 before:content-['']",
          // `bg-white/[0.04]` was invisible against the light theme — the
          // bracketed opacity escaped the earlier white-alpha sweep.
          checked
            ? "border-emerald-400 bg-emerald-600/50"
            : "border-border-strong bg-border-subtle",
        )}
      >
        <span
          className={cn(
