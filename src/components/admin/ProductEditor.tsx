...
export type ProductDraft = {
  ...
  description: string;
  story: string;
  /** Rupees, for the form. Converted to paise server-side. */
  price: string;
...
export function ProductEditor({
  ...
  useEffect(() => {
    ...
    const location: Record<string, Tab> = {
      name: "General",
      slug: "General",
      categoryId: "General",
      ...
      <div className="min-w-0">
        ...
        <div className="mt-7">
          ...
          <Panel active={tab === "General"}>
            <FieldGroup title="Identity" description="What the product is called and where it lives.">
              <Input
                label="Product name"
                name="name"
                required
          ...
          <Panel active={tab === "Media"}>
            {draft.id ? (
              ...
              <p className="text-sm text-cream-400">Save the product first to add images.</p>
            )}
          </Panel>
            ...
            <FieldGroup title="Preview" description="Roughly how this appears in a search result.">
              <div className="max-w-[38rem] border border-white/12 bg-white/[0.02] p-5">
                ...
                <p className="mt-1 line-clamp-2 text-sm text-cream-400">
                  {seoDescription || draft.shortDescription || "Add a meta description."}
                </p>
              </div>
            </FieldGroup>
          </Panel>
          ...
          <Panel active={tab === "Advanced"}>
            ...
            {draft.id ? (
              <FieldGroup title="Related content" description="Managed from their own screens.">
            ...
            <dl className="grid gap-3 text-sm">
              ...
              <div className="flex items-baseline justify-between gap-3">
                ...
                <dd>
                  <Pill
                    tone={
                      variants.some((v) => v.onHand - v.reserved > 0) ? "success" : "danger"
                    }
                  >
                    {variants.reduce((sum, v) => sum + Math.max(0, v.onHand - v.reserved), 0)}
                  </Pill>
                </dd>
              </div>
            </dl>
...
function Panel({ active, children }: { active: boolean; children: React.ReactNode }) {
  return (
    <div role="tabpanel" hidden={!active} className={cn("grid gap-9", !active && "hidden")}>
      {children}
    </div>
  );
}
