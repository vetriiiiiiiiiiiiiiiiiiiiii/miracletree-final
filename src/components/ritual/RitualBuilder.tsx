 ...
 * The ritual builder.
 *
 * A daily routine assembled a step at a time — morning, nourishment, something
 * to carry, something for after. Each step is one category of the catalogue, so
 * the steps are structure while everything inside them is a database row.
 *
 * Two things it deliberately does not do. It does not compute a discount: the
 * saving shown is evaluated on the server from the same coupon the cart will
 * apply, and when no promotion is running it shows none rather than inventing
 * one. And it does not require a full set — every step can be skipped, because
 * a builder that refuses to proceed is a form, not a suggestion.
 */
...
export function RitualBuilder({
  ...
  steps: RitualStep[];
  /** The live promotion, or null when none is running. */
  promo: { code: string; percent: number; minSubtotal: number; maxDiscount: number | null } | null;
}) {
  const [picked, setPicked] = useState<Record<string, RitualChoice | undefined>>({});
  ...
  const chosen = useMemo(
    () => steps.map((s) => picked[s.key]).filter((c): c is RitualChoice => Boolean(c)),
    [picked, steps],
  );
  ...
  const toGo = promo && subtotal > 0 && subtotal < promo.minSubtotal
    ? promo.minSubtotal - subtotal
    : 0;

  const choose = (stepKey: string, choice: RitualChoice) => {
    setError(null);
    setPicked((prev) => ({
      ...prev,
          ...
          {chosen.length === 0 ? (
            ...
            <ul className="mt-5 grid gap-3">
              {steps.map((step) => {
                ...
                return (
                  <li key={step.key} className="flex items-baseline justify-between gap-3">
