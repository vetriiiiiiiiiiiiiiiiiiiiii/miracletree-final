...
export function WishlistGrid({ items }: { items: WishlistEntry[] }) {
  ...
  return (
    <>
      {error ? (
        <p className="mt-6 border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-[#f0b3b0]" role="alert">
          {error}
        </p>
      ) : null}
            ...
            <div className="flex flex-1 flex-col pt-5">
              ...
              <div className="mt-auto flex flex-wrap items-center gap-3 pt-5">
                {item.inStock ? (
                  ...
                  <span className="text-xs text-cream-400">Back after the next harvest</span>
                )}
