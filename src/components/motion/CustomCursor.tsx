...
/**
 * The cursor is a moringa leaflet, and only a moringa leaflet.
 *
 * No ring, no box, no label, no trail. State is expressed through the leaf:
 *
 *   at rest                  one leaflet, its tip on the pointer
 *   over a link              it lifts and turns very slightly toward the target
 *   over a product or image  two more leaflets unfurl, making a compound leaf
 *   pressed                  it presses in, pivoting on its own tip
 *   over text                the native caret returns; a leaf cannot show a column
 *
 * The tip is the hotspot. Everything scales and rotates about it, so however the
 * leaf moves the point stays exactly where the pointer is — which is the whole
 * reason this is a leaflet and not the seed the first version drew. A round seed
 * gave the pointer no point, and at cursor size it read as an apple.
 *
 * Three rules keep it from becoming a usability problem: it activates only on a
 * fine pointer, only when motion is allowed, and it hides the native cursor via
 * `data-cursor-mode` on <html>, so a JS failure leaves the real pointer intact.
 */
type CursorState = "default" | "link" | "view" | "drag" | "image" | "hidden";
...
const TILT: Record<CursorState, number> = {
  default: 0,
  link: -8,
  view: -12,
  drag: -12,
...
const SPRIGS: CursorState[] = ["view", "drag", "image"];

export function CustomCursor({ hasPhoto = false }: { hasPhoto?: boolean }) {
  const leafRef = useRef<HTMLDivElement>(null);
  const sprigRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const setSway = gsap.quickTo(leaf, "rotation", { duration: 0.55, ease: "power2.out" });
    ...
    const resolveState = (target: EventTarget | null): CursorState => {
      ...
      if (declared) {
        const value = declared.dataset.cursor as CursorState | undefined;
    ...
    <div aria-hidden className="pointer-events-none fixed inset-0 z-[9999] hidden md:block">
      ...
      >
        {hasPhoto ? (
          ...
          <img src="/cursor/seed.webp" alt="" className="h-full w-full object-contain" />
        ) : (
