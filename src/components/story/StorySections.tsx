 ...
 *
 * Everything here is rendered on paper rather than on the site's usual dark
 * ground, and every factual claim carries the source it came from. For a food
 * brand that is not decoration: an award or certification a reader cannot trace
 * is a liability, so the citation is part of the component, not an afterthought.
 */
...
function Cite({ source, url }: { source: string | null; url: string | null }) {
...
export function Timeline({ milestones }: { milestones: MilestoneEntry[] }) {
          ...
          {milestones.map((entry, index) => (
            ...
            >
              ...
              <div className="md:pl-6">
                <h3 className="text-[1.2rem] leading-snug text-[#23301f]">{entry.title}</h3>
                {entry.body ? (
...
export function Accolades({ accolades }: { accolades: AccoladeEntry[] }) {
  if (!accolades.length) return null;

  const groups = ["award", "certification", "recognition"].map((kind) => ({
    kind,
...
export function Credits({ credits }: { credits: CreditEntry[] }) {
  if (!credits.length) return null;

  const groups = ["team", "partner", "grower", "design"].map((group) => ({
    group,
                ...
                <div className="relative aspect-4/3 overflow-hidden bg-[#e6e0cc]">
                  {note.heroImageUrl ? (
                    ...
                    <div className="grid h-full place-items-center">
                      <BotanicalPlate
                        subject={(["leaf", "flower", "pod"] as const)[index % 3]!}
