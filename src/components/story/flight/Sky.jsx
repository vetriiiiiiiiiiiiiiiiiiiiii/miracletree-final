...
export function Sky({ travelRef, totalTravel }) {
...
function Drifting({ datum, travelRef }) {
  ...
  useFrame((state) => {
    ...
    const drift =
      Math.sin(time * datum.driftSpeed + datum.timeOffset) * datum.driftAmount;
    const bob =
      Math.cos(time * datum.driftSpeed * 0.7 + datum.timeOffset) * datum.bobAmount;
    mesh.position.x = datum.x + drift + evasion * MAX_EVASION * dirX;
    mesh.position.y = datum.y + bob;
    mesh.rotation.z =
      datum.tilt + Math.sin(time * datum.driftSpeed + datum.timeOffset) * 0.12;
  });
  return (
    <mesh
      ref={ref}
