...
function Flight({ count, scrollRef, trackRef, totalTravel, itemRefs, onIndex }) {
  ...
  useFrame((_state, delta) => {
    ...
    if (flying.current) {
      ...
      const targetPitch =
        Math.sin(chunkProgress * Math.PI * 4) * PITCH_AMPLITUDE * ease;
...
export function StoryFlight({ milestones }) {
        ...
        <div className="pointer-events-none absolute inset-0">
          {milestones.map((m, i) => (
            ...
            >
              ...
              <span
                aria-hidden
                className="mx-auto mt-5 block h-px w-40 bg-[#5d7150]/40"
              />
              {m.body ? (
