 ...
 * products, the certifications. The density comes from cells being full rather
 * than from objects scattered over a background.
 *
 * All of it degrades. `prefers-reduced-motion` disables the entrance and the
 * hero renders as a finished composition rather than an empty one waiting for
 * JavaScript.
 */
        ...
        >
          {NUMBERS.map((n) => (
            <div key={n.label}>
              ...
              <dt className="mt-1 text-[0.7rem] leading-snug text-cream-400">{n.label}</dt>
            </div>
          ))}
        </dl>
