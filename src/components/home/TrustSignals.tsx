 ...
 *
 * Every line here is a certificate number, an issuing body, a date or a named
 * institution that has been to the site. Nothing says "trusted by thousands"
 * or "doctor approved", because those are assertions rather than evidence and
 * this section only works if a sceptical buyer can check it.
 *
 * On the same principle, the people below are described as having *visited*,
 * which is what the company's own captions record. A visit is not an
 * endorsement and is not presented as one.
 */

export type TrustAccolade = {
  title: string;
  issuer: string | null;
  year: string | null;
};
...
export function TrustSignals({ accolades }: { accolades: TrustAccolade[] }) {
          ...
          <Floating>
            <div className="border border-border-subtle bg-ink-800/60 p-7">
              ...
              <p className="mt-3 text-[0.9rem] leading-relaxed text-cream-300">
                ...
                scientists who have been to the Milakaranai site or met the team.
              </p>
