...
export function CheckoutForm({
  ...
  const trackPurchase = (orderNumber) => {
    analytics.purchase({
  ...
  return (
    <>
      ...
      <form
        onSubmit={onSubmit}
        className="grid gap-12 lg:grid-cols-[1.35fr_1fr] lg:gap-16"
      >
        ...
        <aside className="lg:sticky lg:top-28 lg:self-start">
          <div className="border border-border-subtle bg-white/[0.02] p-6 md:p-8">
            ...
            >
              {method === "cod"
                ? "Place order"
                : `Pay ${formatPrice(cart.totals.grandTotal)}`}
            </Button>
...
function PaymentOption({ checked, onSelect, title, body }) {
  return (
    <label
      className={cn(
        "flex cursor-pointer gap-3 border p-4 transition-colors",
