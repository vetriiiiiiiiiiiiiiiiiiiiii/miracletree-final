...
type CartContextValue = {
  ...
  add: (input: {
    ...
    variantId: string;
    quantity?: number;
    /** Analytics only — never used for pricing. */
    meta: { name: string; variantName: string; price: number; category?: string };
  }) => Promise<boolean>;
...
export function CartProvider({ cart, children }: { cart: CartView; children: ReactNode }) {
  ...
  const remove = useCallback(
    async (itemId: string) => setQuantity(itemId, 0),
    [setQuantity],
  );
  ...
  const value = useMemo(
    ...
    [cart, isOpen, pending, open, close, add, setQuantity, remove, applyCoupon, removeCoupon],
  );
