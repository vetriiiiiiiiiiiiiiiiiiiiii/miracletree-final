  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(async () => {
      ...
      try {
        ...
        if (error.name !== "AbortError") setHits([]);
      } finally {
        setLoading(false);
      }
    }, 220);
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-4">
          ...
          <input
            ...
            aria-activedescendant={
              results[active] ? `search-hit-${results[active].id}` : undefined
            }
            className="w-full bg-transparent py-3 text-lg text-cream-50 placeholder:text-cream-400 focus:outline-none md:text-xl"
          />
          {loading ? (
            <span
              className="h-4 w-4 shrink-0 animate-spin rounded-full border border-cream-400 border-t-transparent"
              aria-hidden
            />
          ) : null}
          <button
                  ...
                  <li key={`${hit.kind}-${hit.id}`}>
                    <button
                      ...
                      className={cn(
                        ...
                        index === active
                          ? "border-border-subtle bg-white/[0.05]"
                          : "hover:bg-white/[0.03]",
                      )}
                    >
                      ...
                      {hit.price !== null ? (
                        ...
                        <span className="eyebrow shrink-0 text-cream-400">
                          {hit.kind}
                        </span>
                      )}
                    </button>
