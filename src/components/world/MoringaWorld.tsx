...
export function MoringaWorld({
   ...
   * pinned to the viewport for the rest of the document and draw on top of
   * every section below the world.
   */
  ...
  useEffect(() => {
    ...
    let scene: import("@/lib/world/MoringaScene").MoringaScene | undefined;
    let cancelled = false;
    const cleanups: (() => void)[] = [];
    ...
    import("@/lib/world/MoringaScene")
      .then(({ MoringaScene }) => {
        ...
        const observer = new IntersectionObserver(
          ([entry]) => scene?.setRunning(Boolean(entry?.isIntersecting) && !document.hidden),
          { threshold: 0 },
        );
